...
<?php
   $query = "select id from repository order by $order $direction limit $begin, ".($pageSize + 1);
   if (!ereg('[^A-Za-z0-9]', $search)) {
      $query = "select id from repository where title like '%$search%' or description like '%$search%' or author like '%$search%' or category like '%$search%' order by $order $direction limit $begin, ".($pageSize + 1);
   }
...
<?php
      ...
      if ($page > 0) {
?>
                              <a href="repository.php?page=<? echo($page - 1); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
?>
                              <a href="repository.php?page=<? echo($i); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      ...
      if (($num > 5) && ($page < $num - 1)) {
?>
                              <a href="repository.php?page=<? echo(intval($num-0.01)); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      $query_feed = "select id, title from feed order by id desc";
...
<?php
   if ($action == "modify"){
      $query = "select feedID, title, description, publishDate, author, category, guid from repository where id='$id[0]'";
      $result = mysql_query($query) or die("Invalid query: " . mysql_error());
      $row = mysql_fetch_row($result) or die("Could not retrieve row: " . mysql_error());
?>
   <div id="content">
...
?>
               ...
               <tr>
                  <td class="title">GUID</td>
                  <td class="dark">
                     <input type="text" size="40" name="guid" value="<?php echo($row[6]); ?>" />
                  </td>
               </tr>
...
<?php
      foreach ($id as $idx) {
         $query = "select feedID, title, description, publishDate, author, category, guid from repository where id='$idx'";
         $result = mysql_query($query) or die("Invalid query: " . mysql_error());
         $row = mysql_fetch_row($result) or die("Could not retrieve row: " . mysql_error());
?>
               <tr class="dark">
...
?>
               <tr><td class="buttons" colspan="10"><input class="image_button" type="image" src="images/apagar.png" value="Apagar" onclick="javascript: form.submit;" /><input class="image_button" type="image" src="images/cancelar.png" value="Cancelar" onclick="javascript:document.location='repository.php';return false;" /></td></tr>
            </table>
         </form>
      </div>
   </div>
