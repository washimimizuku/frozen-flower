...
<?php
   $query = "select id, name, stub, active, feed from category order by $order $direction limit $begin, ".($pageSize + 1);
   if (!ereg('[^A-Za-z0-9]', $search)) {
      $query = "select id, name, stub, active, feed from category where name like '%$search%' order by $order $direction limit $begin, ".($pageSize + 1);
   }
...
?>
                  ...
                  <td><?php if ($row[3] == 1) { echo('Sim'); } else { echo('Não'); } ?></td>                  <td><?php if ($row[4] == 1) { echo('Sim'); } else { echo('Não'); } ?></td>
...
<?php
      ...
      if ($page > 0) {
?>
                              <a href="category.php?page=<? echo($page - 1); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
?>
                              <a href="category.php?page=<? echo($i); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      ...
      if (($num > 5) && ($page < $num - 1)) {
?>
                              <a href="category.php?page=<? echo(intval($num-0.01)); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      $query_category = "select id, name from category order by id desc";
...
?>
               ...
               <tr>
                  <td class="title">Feed</td>
                  <td class="dark">
                     <input type="radio" name="feed" value="1"<?php if ($row[4] == 1) { echo(' checked'); } ?> /> Sim <input type="radio" name="feed" value="0"<?php if ($row[4] == 0) { echo(' checked'); } ?> /> Não
                  </td>
               </tr>
