...
<?php
   $query = "select id from enclosure order by $order $direction limit $begin, ".($pageSize + 1);
   if (!ereg('[^A-Za-z0-9]', $search)) {
      $query = "select id from enclosure where  order by $order $direction limit $begin, ".($pageSize + 1);
   }
...
<?php
      ...
      if ($page > 0) {
?>
                              <a href="enclosure.php?page=<? echo($page - 1); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
?>
                              <a href="enclosure.php?page=<? echo($i); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      ...
      if (($num > 5) && ($page < $num - 1)) {
?>
                              <a href="enclosure.php?page=<? echo(intval($num-0.01)); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      $query_repository = "select id, title from repository order by id desc";
