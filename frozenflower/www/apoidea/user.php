         ...
         <div class="tabela">
            <table border="0" cellspacing="0">
               <tr class="title">
                  ...
                  <td>ID</td>
                  <td>Grupo <a href="user.php?order=tribeID&direction=asc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/up.png" /></a><a href="user.php?order=tribeID&direction=desc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/down.png" /></a></td>                  <td>Nome <a href="user.php?order=name&direction=asc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/up.png" /></a><a href="user.php?order=name&direction=desc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/down.png" /></a></td>                  <td>Username <a href="user.php?order=username&direction=asc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/up.png" /></a><a href="user.php?order=username&direction=desc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/down.png" /></a></td>
                  <td></td>
                  <td></td>
               </tr>
<?php
   $query = "select id, id, tribeID, name, username from user order by $order $direction limit $begin, ".($pageSize + 1);
   if (!ereg('[^A-Za-z0-9]', $search)) {
      $query = "select id, id, tribeID, name, username from user where tribeID like '%$search%' or name like '%$search%' or username like '%$search%' order by $order $direction limit $begin, ".($pageSize + 1);
   }
...
<?php
      $query_tribe = "select name from tribe where id=$row[2]";
...
<?php
      ...
      if ($page > 0) {
?>
                              <a href="user.php?page=<? echo($page - 1); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
?>
                              <a href="user.php?page=<? echo($i); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      ...
      if (($num > 5) && ($page < $num - 1)) {
?>
                              <a href="user.php?page=<? echo(intval($num-0.01)); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      $query_tribe = "select id, name from tribe order by id desc";
      $result_tribe = mysql_query($query_tribe) or die("Invalid query: " . mysql_error());
...
?>
               ...
               <tr>
                  <td class="title">Password (confirmar)</td>
                  <td class="dark">
