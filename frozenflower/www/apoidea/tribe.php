         ...
         <div class="tabela">
            <table border="0" cellspacing="0">
               <tr class="title">
                  ...
                  <td>ID</td>
                  <td>Nome <a href="tribe.php?order=name&direction=asc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/up.png" /></a><a href="tribe.php?order=name&direction=desc<?php if ($search) { echo "&search=$search";} ?>"><img class="small_button" src="images/down.png" /></a></td>
                  <td></td>
                  <td></td>
               </tr>
<?php
   $query = "select id, id, name from tribe order by $order $direction limit $begin, ".($pageSize + 1);
   if (!ereg('[^A-Za-z0-9]', $search)) {
      $query = "select id, id, name from tribe where name like '%$search%' order by $order $direction limit $begin, ".($pageSize + 1);
   }
...
<?php
      ...
      if ($page > 0) {
?>
                              <a href="tribe.php?page=<? echo($page - 1); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
?>
                              <a href="tribe.php?page=<? echo($i); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
<?php
      ...
      if (($num > 5) && ($page < $num - 1)) {
?>
                              <a href="tribe.php?page=<? echo(intval($num-0.01)); ?>&order=<? echo($order); ?>&direction=<? echo($direction); ?>&search=<?
...
?>
   <div id="content">
      <h2><a href="tribe.php">Grupo</a> > Criar Grupo</h2>
<?php
	if ($msg) {
?>
...
?>
   <div id="content">
      <h2><a href="tribe.php">Grupo</a> > Alterar Grupo</h2>
      <div class="tabela">
         <form name="form" method="post" action="tribe_submit.php">
...
?>
   <div id="content">
      <h2><a href="tribe.php">Grupo</a> > Apagar Grupo</h2>
      <div class="tabela">
